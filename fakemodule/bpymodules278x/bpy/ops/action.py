...
def clickselect(*args, extend=False, column=False, channel=False):
...
def duplicate_move(*args, ACTION_OT_duplicate=None, TRANSFORM_OT_transform=None):
    ...
    :param ACTION_OT_duplicate: Duplicate Keyframes, Make a copy of all selected keyframes
        (type: ACTION_OT_duplicate, (optional))
    :type ACTION_OT_duplicate: ACTION_OT_duplicate
...
def keyframe_insert(*args, type='ALL'):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param type: Type
        (type: enum in ['ALL', 'SEL', 'GROUP'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def stash(*args, create_new=True):
...
def unlink(*args, force_delete=False):
