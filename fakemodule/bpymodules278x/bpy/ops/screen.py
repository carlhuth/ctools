def actionzone(*args, modifier=0):
...
def animation_cancel(*args, restore_frame=True):
...
def animation_play(*args, reverse=False, sync=False):
    """Play animation
...
def area_join(*args, min_x=-100, min_y=-100, max_x=-100, max_y=-100):
...
def area_move(*args, x=0, y=0, delta=0):
...
def area_split(*args, direction='HORIZONTAL', factor=0.5, mouse_x=-100, mouse_y=-100):
    ...
    :param direction: Direction
        (type: enum in ['HORIZONTAL', 'VERTICAL'], (optional))
    :type direction: str
    :param factor: Factor
...
def keyframe_jump(*args, next=True):
...
def marker_jump(*args, next=True):
...
def repeat_history(*args, index=0):
...
def screen_full_area(*args, use_hide_panels=False):
...
def space_context_cycle(*args, direction='NEXT'):
    ...
    :param direction: Direction, Direction to cycle through
        (type: enum in ['PREV', 'NEXT'], (optional))
    :type direction: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
