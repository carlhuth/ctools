...
def constraint_operation(*args, type=’ENABLE’):
    ...
    :param type: Constraint Operation
        (type: enum in ['ENABLE', 'DISABLE', 'DELETE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def data_operation(*args, type=’SELECT’):
    ...
    :param type: Data Operation
        (type: enum in ['SELECT', 'DESELECT', 'HIDE', 'UNHIDE', 'SELECT_LINKED'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def drivers_delete_selected(*args):
...
def group_operation(*args, type=’UNLINK’):
    ...
    :param type: Group Operation
        ...
        * 'RENAME': Rename.
        (type: enum in ['UNLINK', 'LOCAL', 'LINK', 'DELETE', 'REMAP', 'INSTANCE', 'TOGVIS', 'TOGSEL', 'TOGREN', 'RENAME'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def id_operation(*args, type=’UNLINK’):
    ...
    :param type: ID data Operation
        ...
        * 'SELECT_LINKED': Select Linked.
        (type: enum in ['UNLINK', 'LOCAL', 'SINGLE', 'DELETE', 'REMAP', 'ADD_FAKE', 'CLEAR_FAKE', 'RENAME', 'SELECT_LINKED'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def id_remap(*args, id_type=’OBJECT’, old_id=”, new_id=”):
    ...
    :param id_type: ID Type
        (type: enum in ['ACTION', 'ARMATURE', 'BRUSH', 'CAMERA', 'CACHEFILE', 'CURVE', 'FONT', 'GREASEPENCIL', 'GROUP', 'IMAGE', 'KEY', 'LAMP', 'LIBRARY', 'LINESTYLE', 'LATTICE', 'MASK', 'MATERIAL', 'META', 'MESH', 'MOVIECLIP', 'NODETREE', 'OBJECT', 'PAINTCURVE', 'PALETTE', 'PARTICLE', 'SCENE', 'SCREEN', 'SOUND', 'SPEAKER', 'TEXT', 'TEXTURE', 'WINDOWMANAGER', 'WORLD'], (optional))
    :type id_type: str
...
def item_activate(*args, extend=True, recursive=False):
...
def item_openclose(*args, all=True):
...
def keyingset_add_selected(*args):
...
def keyingset_remove_selected(*args):
...
def modifier_operation(*args, type=’TOGVIS’):
    ...
    :param type: Modifier Operation
        (type: enum in ['TOGVIS', 'TOGREN', 'DELETE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def object_operation(*args, type=’SELECT’):
    ...
    :param type: Object Operation
        ...
        * 'TOGREN': Toggle Renderable.
        * 'RENAME': Rename.
        (type: enum in ['SELECT', 'DESELECT', 'SELECT_HIERARCHY', 'DELETE', 'DELETE_HIERARCHY', 'REMAP', 'TOGVIS', 'TOGSEL', 'TOGREN', 'RENAME'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def orphans_purge(*args):
    """Clear all orphaned data-blocks without any users from the file (cannot be undone)
...
def select_border(*args, gesture_mode=0, xmin=0, xmax=0, ymin=0, ymax=0):
...
def show_one_level(*args, open=True):
