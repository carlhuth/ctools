...
def active_frames_delete_all(*args):
...
def blank_frame_add(*args, all_layers=False):
    """Add a new frame with nothing in it on the current frame. If there is already a frame, all existing frames are shifted one frame later
...
def brush_paint(*args, stroke=None, wait_for_input=True):
...
def convert(*args, type='PATH', use_normalize_weights=True, radius_multiplier=1.0, use_link_strokes=True, timing_mode='FULL', frame_range=100, start_frame=1, use_realtime=False, end_frame=250, gap_duration=0.0, gap_randomness=0.0, seed=0, use_timing_data=False):
    ...
    :param timing_mode: Timing Mode, How to use timing data stored in strokes
        ...
        * 'CUSTOMGAP': Custom Gaps, Use the original timing, but with custom gap lengths (in frames).
        (type: enum in ['NONE', 'LINEAR', 'FULL', 'CUSTOMGAP'], (optional))
    :type timing_mode: str
...
def draw(*args, mode='DRAW', stroke=None, wait_for_input=True):
    ...
    :param mode: Mode, Way to interpret mouse movements
        ...
        * 'ERASER': Eraser, Erase Grease Pencil strokes.
        (type: enum in ['DRAW', 'DRAW_STRAIGHT', 'DRAW_POLY', 'ERASER'], (optional))
    :type mode: str
    :param stroke: Stroke
        (type: bpy_prop_collection of OperatorStrokeElement, (optional))
    :type stroke: bpy_prop_collection
...
def duplicate_move(*args, GPENCIL_OT_duplicate=None, TRANSFORM_OT_translate=None):
    ...
    :param GPENCIL_OT_duplicate: Duplicate Strokes, Duplicate the selected Grease Pencil strokes
        (type: GPENCIL_OT_duplicate, (optional))
    :type GPENCIL_OT_duplicate: GPENCIL_OT_duplicate
...
def interpolate(*args, shift=0.0):
...
def layer_isolate(*args, affect_visibility=False):
...
def palettecolor_hide(*args, unselected=False):
...
def palettecolor_isolate(*args, affect_visibility=False):
...
def palettecolor_lock_all(*args):
...
def palettecolor_unlock_all(*args):
...
def paste(*args, type='COPY'):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param type: Type
        (type: enum in ['COPY', 'MERGE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def reproject(*args, type='PLANAR'):
    ...
    :param type: Projection Type
        ...
        * 'SURFACE': Surface, Reproject the strokes on to the scene geometry, as if drawn using 'Surface' placement.
        (type: enum in ['PLANAR', 'SURFACE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def select(*args, extend=False, deselect=False, toggle=False, entire_strokes=False, location=(0, 0)):
    ...
    :param location: Location, Mouse location
        (type: int array of 2 items in [-inf, inf], (optional))
...
def select_first(*args, only_selected_strokes=False, extend=False):
...
def select_grouped(*args, type='LAYER'):
    ...
    :param type: Type
        ...
        * 'COLOR': Color, Shared colors.
        (type: enum in ['LAYER', 'COLOR'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def select_last(*args, only_selected_strokes=False, extend=False):
...
def selection_opacity_toggle(*args):
...
def snap_cursor_to_selected(*args):
...
def snap_to_cursor(*args, use_offset=True):
...
def stroke_apply_thickness(*args):
...
def stroke_arrange(*args, direction='UP'):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param direction: Direction
        (type: enum in ['UP', 'DOWN', 'TOP', 'BOTTOM'], (optional))
    :type direction: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def stroke_cyclical_set(*args, type='TOGGLE'):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param type: Type
        (type: enum in ['CLOSE', 'OPEN', 'TOGGLE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def stroke_join(*args, type='JOIN', leave_gaps=False):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param type: Type
        (type: enum in ['JOIN', 'JOINCOPY'], (optional))
    :type type: str
...
def stroke_subdivide(*args, number_cuts=1):
