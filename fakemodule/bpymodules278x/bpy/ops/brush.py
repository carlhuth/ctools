...
def scale_size(*args, scalar=1.0):
...
def stencil_control(*args, mode='TRANSLATION', texmode='PRIMARY'):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param mode: Tool
        (type: enum in ['TRANSLATION', 'SCALE', 'ROTATION'], (optional))
    :type mode: str
    :param texmode: Tool
        (type: enum in ['PRIMARY', 'SECONDARY'], (optional))
    :type texmode: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def stencil_fit_image_aspect(*args, use_repeat=True, use_scale=True, mask=False):
...
def stencil_reset_transform(*args, mask=False):
...
def uv_sculpt_tool_set(*args, tool='PINCH'):
    ...
    :param tool: Tool
        ...
        * 'GRAB': Grab, Grab UVs.
        (type: enum in ['PINCH', 'RELAX', 'GRAB'], (optional))
    :type tool: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
