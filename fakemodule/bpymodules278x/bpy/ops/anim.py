def change_frame(*args, frame=0.0, snap=False):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param frame: Frame
        (type: float in [-500000, 500000], (optional))
    :type frame: float
    :param snap: Snap
...
def channel_select_keys(*args, extend=False):
...
def channels_click(*args, extend=False, children_only=False):
...
def channels_collapse(*args, all=True):
...
def channels_editable_toggle(*args, mode=’TOGGLE’, type=’PROTECT’):
    ...
    :param args: (override_context, execution_context, undo)
        ...
        undo (bool)
    :param mode: Mode
        (type: enum in ['TOGGLE', 'DISABLE', 'ENABLE', 'INVERT'], (optional))
    :type mode: str
    :param type: Type
        (type: enum in ['PROTECT', 'MUTE'], (optional))
    :type type: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def channels_expand(*args, all=True):
...
def channels_fcurves_enable(*args):
...
def channels_select_all_toggle(*args, invert=False):
...
def channels_select_border(*args, gesture_mode=0, xmin=0, xmax=0, ymin=0, ymax=0, extend=True):
...
def clear_useless_actions(*args, only_unused=True):
...
def driver_button_remove(*args, all=True):
...
def keyframe_clear_button(*args, all=True):
...
def keyframe_delete_button(*args, all=True):
...
def keyframe_insert_button(*args, all=True):
...
def keying_set_path_remove(*args):
...
def keyingset_button_add(*args, all=True):
...
def keyingset_button_remove(*args):
...
def previewrange_set(*args, xmin=0, xmax=0, ymin=0, ymax=0):
...
def update_animated_transform_constraints(*args, use_convert_to_radians=True):
