def add_marker(*args, location=(0.0, 0.0)):
...
def add_marker_move(*args, CLIP_OT_add_marker=None, TRANSFORM_OT_translate=None):
    ...
    :param CLIP_OT_add_marker: Add Marker, Place new marker at specified location
        (type: CLIP_OT_add_marker, (optional))
    :type CLIP_OT_add_marker: CLIP_OT_add_marker
...
def add_marker_slide(*args, CLIP_OT_add_marker=None, TRANSFORM_OT_translate=None):
...
def apply_solution_scale(*args, distance=0.0):
...
def clean_tracks(*args, frames=0, error=0.0, action='SELECT'):
    ...
    :param action: Action, Cleanup action to execute
        ...
        * 'DELETE_SEGMENTS': Delete Segments, Delete unclean segments of tracks.
        (type: enum in ['SELECT', 'DELETE_TRACK', 'DELETE_SEGMENTS'], (optional))
    :type action: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def clear_track_path(*args, action='REMAINED', clear_active=False):
    ...
    :param action: Action, Clear action to execute
        ...
        * 'ALL': Clear all, Clear the whole path.
        (type: enum in ['UPTO', 'REMAINED', 'ALL'], (optional))
    :type action: str
...
def detect_features(*args, placement='FRAME', margin=16, threshold=0.5, min_distance=120):
    ...
    :param placement: Placement, Placement for detected features
        ...
        * 'OUTSIDE_GPENCIL': Outside grease pencil, Place markers only outside areas outlined with grease pencil.
        (type: enum in ['FRAME', 'INSIDE_GPENCIL', 'OUTSIDE_GPENCIL'], (optional))
    :type placement: str
    ...
    :param threshold: Threshold, Threshold level to consider feature good enough for tracking
        (type: float in [0.0001, inf], (optional))
...
def dopesheet_select_channel(*args, location=(0.0, 0.0), extend=False):
...
def filter_tracks(*args, track_threshold=5.0):
...
def frame_jump(*args, position='PATHSTART'):
    ...
    :param position: Position, Position to jump to
        ...
        * 'FAILNEXT': Next Failed, Jump to next failed frame.
        (type: enum in ['PATHSTART', 'PATHEND', 'FAILEDPREV', 'FAILNEXT'], (optional))
    :type position: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def graph_center_current_frame(*args):
...
def graph_select(*args, location=(0.0, 0.0), extend=False):
    """Select graph curves
...
def graph_select_border(*args, gesture_mode=0, xmin=0, xmax=0, ymin=0, ymax=0, extend=True):
...
def hide_tracks(*args, unselected=False):
...
def lock_tracks(*args, action='LOCK'):
    ...
    :param action: Action, Lock action to execute
        ...
        * 'TOGGLE': Toggle, Toggle locked flag for selected tracks.
        (type: enum in ['LOCK', 'UNLOCK', 'TOGGLE'], (optional))
    :type action: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def mode_set(*args, mode='TRACKING'):
    ...
    :param mode: Mode
        ...
        * 'MASK': Mask, Show mask editing tools.
        (type: enum in ['TRACKING', 'MASK'], (optional))
    :type mode: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def select_grouped(*args, group='ESTIMATED'):
    ...
    :param group: Action, Clear action to execute
        ...
        * 'FAILED': Failed Tracks, Select all tracks which failed to be reconstructed.
        (type: enum in ['KEYFRAMED', 'ESTIMATED', 'TRACKED', 'LOCKED', 'DISABLED', 'COLOR', 'FAILED'], (optional))
    :type group: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def set_origin(*args, use_median=False):
...
def set_plane(*args, plane='FLOOR'):
    ...
    :param plane: Plane, Plane to be used for orientation
        ...
        * 'WALL': Wall, Set wall plane.
        (type: enum in ['FLOOR', 'WALL'], (optional))
    :type plane: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def set_scale(*args, distance=0.0):
...
def set_solution_scale(*args, distance=0.0):
...
def set_solver_keyframe(*args, keyframe='KEYFRAME_A'):
    ...
    :param keyframe: Keyframe, Keyframe to set
        (type: enum in ['KEYFRAME_A', 'KEYFRAME_B'], (optional))
    :type keyframe: str
    :return: (type: enum set in {'RUNNING_MODAL', 'CANCELLED', 'FINISHED', 'PASS_THROUGH'})
    :rtype: set[str]
    """
...
def set_viewport_background(*args):
...
def slide_marker(*args, offset=(0.0, 0.0)):
    """Slide marker areas
...
def stabilize_2d_rotation_add(*args):
...
def stabilize_2d_rotation_remove(*args):
...
def stabilize_2d_rotation_select(*args):
...
def track_markers(*args, backwards=False, sequence=False):
...
def track_settings_as_default(*args):
...
def track_settings_to_track(*args):
...
def tracking_object_remove(*args):
...
def view_all(*args, fit_view=False):
...
def view_pan(*args, offset=(0.0, 0.0)):
    """Pan the view
...
def view_zoom_in(*args, location=(0.0, 0.0)):
    """Zoom in the view
...
def view_zoom_out(*args, location=(0.0, 0.0)):
    """Zoom out the view
...
def view_zoom_ratio(*args, ratio=0.0):
